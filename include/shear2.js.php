...
try {
    ...
    //Get all the BOPs
    $stmt_bop = $conn->prepare("SELECT BOP.id AS BOP_id, OEM.id as OEM_id, name AS OEM, BOP_model, BOP_closingarea, BOP_closingratio, BOP_taildiameter, BOP_MOPFLPS FROM BOP, OEM WHERE BOP.BOP_OEM_id=OEM.id ORDER BY OEM;");   //Gets all . 
    $stmt_bop->execute();
...
for ($row = 0; $row < count($bop); $row++){
    ...
    if ($bop[$row]["BOP_MOPFLPS"]==null){
        $bop[$row]["BOP_MOPFLPS"]="unavailable";
    }
}
?>
<script>
function BOP_fields() {
    ...
    */
    ...
    var formTailrod,formClosingRatio, formClosingArea, formMOPFLPS, OEM_choice, BOP_choice;
    ...
    if (BOPchoice[0].checked){  //The user has chosen to "Select" a BOP from the database.
            ...
            for (var BOP_row = 0; BOP_row < BOP.length; BOP_row++){
                if (BOP[BOP_row]['OEM_id']===OEM_choice){
                    if((BOP[BOP_row]['BOP_id']===BOP_choice) || (BOP[BOP_row]['BOP_id']!==BOP_choice && first_BOP)){  // adds selected value to the option if it was previously selected.
                        ...
                        formClosingArea = "<div class=\"w3-row\"><div class=\"w3-col "+l3col_class+"\">Closing Area</div><div class=\"w3-col "+m3col_class+"\"><input type=\"text\" class=\"w3-input w3-padding-0\" name=\"closingArea\" id=\"bop_closingarea\" value=\""+BOP[BOP_row]['BOP_closingarea']+"\"/></div><div class=\"w3-col "+r3col_class+"\">in<sup>2</sup></div></div>";
                        formClosingRatio = "<div class=\"w3-row\"><div class=\"w3-col "+l3col_class+"\">Closing Ratio</div><div class=\"w3-col "+m3col_class+"\"><input type=\"text\" name=\"bop_closingratio\" id=\"bop_closingratio\" class=\"w3-input w3-padding-0\" value=\""+BOP[BOP_row]['BOP_closingratio']+"\"/></div></div>";
                        formTailrod = "<div class=\"w3-row\"><div class=\"w3-col "+l3col_class+"\">Tailrod Area</div><div class=\"w3-col "+m3col_class+"\"><input type=\"text\" name=\"TailrodArea\" id=\"bop_trarea\" value='"+BOP[BOP_row]['BOP_tailrodarea']+"' class=\"w3-input w3-padding-0\"/></div><div class=\"w3-col "+r3col_class+"\">in<sup>2</sup></div></div>";
                        formMOPFLPS = "<div class=\"w3-row\"><div class=\"w3-col "+l3col_class+"\">MOPFLPS<span title='Minimum Operationg Pressure For Low Pressure Sealing'><span class='fa fa-info-circle w3-small'></span></span></div><div class=\"w3-col "+m3col_class+"\"><input type=\"text\" name='MOPFLPS' id='bop_MOPFLPS' value='"+BOP[BOP_row]['BOP_MOPFLPS']+"' class=\"w3-input w3-padding-0\"/></div><div class=\"w3-col "+r3col_class+"\">psi</div></div>";
                        BOPproperties = formClosingArea+formClosingRatio+formTailrod+formMOPFLPS;
        ...
        formClosingArea = "<div class=\"w3-row\"><div class=\"w3-col " + l3col_class + "\">Closing Area</div><div class=\"w3-col " + m3col_class + "\"><input type=\"text\" name=\"closingArea\" id=\"bop_closingarea\" class=\"w3-input w3-padding-0\"/></div><div class=\"w3-col " + r3col_class + "\">in<sup>2</sup></div></div>";
        formClosingRatio = "<div class=\"w3-row\"><div class=\"w3-col "+l3col_class+"\">Closing Ratio</div><div class=\"w3-col " + m3col_class + "\"><input type=\"text\" name=\"bop_closingratio\" id=\"bop_closingratio\" class=\"w3-input w3-padding-0\"/></div></div>";
        formTailrod = "<div class=\"w3-row\"><div class=\"w3-col "+l3col_class+"\">Tailrod Area</div><div class=\"w3-col "+m3col_class+"\"><input type=\"text\" name=\"TailrodArea\" id=\"bop_trarea\" class=\"w3-input w3-padding-0\"/></div><div class=\"w3-col "+r3col_class+"\">in<sup>2</sup></div></div>";    
        formMOPFLPS = "<div class=\"w3-row\"><div class=\"w3-col "+l3col_class+"\">MOPFLPS<span title='Minimum Operationg Pressure For Low Pressure Sealing'><span class='fa fa-info-circle w3-small'></span></span></div><div class=\"w3-col "+m3col_class+"\"><input type=\"text\" name='MOPFLPS' id='bop_MOPFLPS' class=\"w3-input w3-padding-0\"/></div><div class=\"w3-col "+r3col_class+"\">psi</div></div>";
        divobj.innerHTML = formClosingArea+formClosingRatio+formTailrod+formMOPFLPS;
    } 
}
</script>
