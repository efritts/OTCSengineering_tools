...
function BOP_fields() {
    ...
    */
    ...
    if(document.contains(document.getElementById('OEM_select'))){  //If an OEM has been selected
	    	...
	    	if(document.contains(document.getElementById('BOP_select'))){  //If a BOP has been selected.  Nested inside the OEM selection b/c a BOP can only be selected if an OEM is selected first.
		    	var e = document.getElementById("BOP_select");
		    	BOP_choice = e.options[e.selectedIndex].value;
			}
	}
    if (BOPchoice[0].checked){  //The user has chosen to "Select" a BOP from the database.
    	
	    //construct the OEM select input box
	    var OEMselect = "<select id=\"OEM_select\" onchange=\"BOP_fields(); displayresults();\">";  //The OEM dropdown will call this function and generate the model dropdown  
	    var BOPselect = "";  
    	...
    	if(document.contains(document.getElementById('OEM_select'))){	    	
	    	var first_BOP = true;  
	    	BOPselect += "<td>Model:</td><td colspan='2'><select id=\"BOP_select\" onchange=\"BOP_fields(); displayresults();\">";
			...
			BOPselect +="</select></td>"
    		//construct the BOP parameters table(Closing area, closing ratio, tail rod) based on model)
    		if(document.contains(document.getElementById('BOP_select'))){
